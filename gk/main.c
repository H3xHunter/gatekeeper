...
setup_gk_instance(unsigned int lcore_id, struct gk_config *gk_conf)
{
	...
	if (instance->acl == NULL) {
		RTE_LOG(ERR, MALLOC,
			"The GK block can't create acl search at lcore %u!\n",
			lcore_id);
	...
	ret = init_mailbox("gk", gk_conf->net->mailbox_max_entries,
		sizeof(struct gk_cmd_entry),
		gk_conf->net->mailbox_mem_cache_size,
...
process_pkts_back(uint8_t port_back, uint8_t port_front,
	...
	unsigned int lcore, struct acl_search *acl, struct gk_config *gk_conf)
{
	/* Get burst of RX packets, from first port of pair. */
	int i;
	int ret;
	...
	struct gk_instance *instance, struct gk_config *gk_conf)
{
	int i;
	int num_cmd;
	struct gk_cmd_entry *gk_cmds[gk_conf->net->mailbox_burst_size];

	/* Load a set of commands from its mailbox ring. */
        num_cmd = mb_dequeue_burst(&instance->mb,
		(void **)gk_cmds, gk_conf->net->mailbox_burst_size);
...
cleanup_gk(struct gk_config *gk_conf)
{
	...
	for (i = 0; i < gk_conf->num_lcores; i++) {
		...
		if (gk_conf->instances[i].acl != NULL)
			destroy_acl_search(gk_conf->instances[i].acl);
